...
| FR-RISK-9 | 失败必须可定位 | 页面未就绪导致的失败必须带上现场证据（页面标题、readyState、loading、可见文字前 160 字），不允许只报"刷不开" |
| FR-RISK-10 | 后台标签受限时自愈 | 连续 2 次页面级失败 → 自动把执行标签切到前台重试一次；同一标签累计导航 40 次 → 主动重建标签（避免长跑退化） |
| FR-RISK-8 | 页面未就绪不得发送 | 内容脚本能应答不代表页面内容可用；详情/打招呼必须等页面脱离加载态（有界等待，上限约 15s），入口出现前不得判定失败；等待超时按"页面级失败"处理而不是工具失败 |
