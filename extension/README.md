...
- `页面加载超时（内容始终处于加载中）标题「…」… 现场：「加载中，请稍候」` → 该岗位页面没加载出来：
  Agent 连续 2 次遇到这种情况会**自动把执行标签切到前台**重试；若页面结构真的变了才需要重新校准 `content.js`
- `连续 3 个岗位页面无法完成操作` → 说明多个岗位页面都刷不开（常见于后台标签被节流、或平台限流）。
  建议把 BOSS 标签保持在前台、或稍后 Resume 再试
